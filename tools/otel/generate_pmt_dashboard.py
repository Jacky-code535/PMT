#!/usr/bin/env python3
"""Generate the provisioned Grafana dashboard for Intel PMT Redfish metrics."""
...
sel = 'RedfishEndpoint="$endpoint",DeviceId=~"$device",AccessId=~"$access"'
temp = f'{{__name__=~".*_temp_c[0-9]+_temp_celcius",{sel}}}'
...
add_stat("Data loss Δ (5m)", "最近五分钟 aggregator data-loss gauge 的正向增量。", 20, 4,
         f'sum(clamp_min(delta(agg_data_loss_count_agg_data_loss_count{{{sel}}}[5m]),0))', unit="locale",
...
add_timeseries("Prometheus scrape duration", "Exporter scrape 耗时持续升高表示 Collector/Prometheus 压力或 series 增长。", 8, 8, 7, [
    target('scrape_duration_seconds{job="otel-pmt"}', "scrape seconds", "A"),
], unit="s", minimum=0)
add_timeseries("Data-loss trend by source", "累计值非零不一定是当前故障；重点观察曲线是否继续增加。", 16, 8, 7, [
    target(f'agg_data_loss_count_agg_data_loss_count{{{sel}}}', "dev{{DeviceId}} / access{{AccessId}}", "A")
], unit="locale", minimum=0)
advance(7)

row("02 · Thermal / 全 Core 热状态", "显示所有 core，而不是只固定 core 0。使用 Device/Access 变量缩小范围。")
core_labeled_temp = f'label_replace({temp},"core","$1","__name__",".*_temp_c([0-9]+)_temp_celcius")'
...
add_timeseries("Selected Core $core temperature", "由顶部 Core 变量选择逻辑 core；可能因多个 Device/Access 产生多条曲线。", 12, 12, 7, [
    target(f'{{__name__=~".*_temp_c$core_temp_celcius",{sel}}}', "D{{DeviceId}}/A{{AccessId}}")
], unit="celsius", minimum=0, maximum=110)
advance(7)
...
add_timeseries("Selected Core $core usage rate", "选定 core 的 usage 5 分钟变化率。", 12, 12, 9, [
    target(f'clamp_min(delta(c$core_usage_meter_core_usage_total{{{sel}}}[5m])/300,0)', "D{{DeviceId}}/A{{AccessId}}")
], unit="ops", minimum=0)
advance(9)
...
for x, kind, title_text in [(0, "freq", "Frequency residency"), (8, "temp", "Temperature residency"), (16, "volt", "Voltage residency")]:
    expr = (f'sum by(bucket)(rate(label_replace({{__name__=~"c$core_{kind}_hist_r[0-9]+_second_total",{sel}}},'
            f'"bucket","$1","__name__","c$core_{kind}_hist_(r[0-9]+)_second_total")[5m:]))')
    add_timeseries(f"{title_text} · Core $core", "Bucket 的准确物理范围见 metric catalog 中对应 HELP。", x, 8, 9,
                   [target(expr, "{{bucket}}")], unit="ops", minimum=0, stacking="normal", legend_place="right")
advance(9)

row("05 · Memory, CHA & accelerator / 内存·缓存代理·QAT", "RDT MBM/CMT、CHA enable mask 与 QAT telemetry。大量同类 metrics 采用 Top-K，避免一次绘制数千条曲线。")
mbm_total = f'{{__name__=~"cha[0-9]+_rmid_.*_rdt_mbm_total_.*",{sel}}}'
mbm_local = f'{{__name__=~"cha[0-9]+_rmid_.*_rdt_mbm_local_.*",{sel}}}'
cmt = f'{{__name__=~"cha[0-9]+_rmid_.*_rdt_cmt_.*",{sel}}}'
add_timeseries("Top RDT MBM total activity", "CHA/RMID total memory transaction gauge 的 5 分钟正向变化率。", 0, 12, 9, [
    target(f'topk(20,clamp_min(delta(label_replace({mbm_total},"channel","$1","__name__",".*_(cha[0-9]+_rmid[0-9]+)_rdt_mbm_total")[5m:])/300,0))', "{{channel}} · D{{DeviceId}}/A{{AccessId}}")
], unit="ops", minimum=0)
add_timeseries("Top RDT MBM local activity", "CHA/RMID local memory transaction gauge 的 5 分钟正向变化率。", 12, 12, 9, [
    target(f'topk(20,clamp_min(delta(label_replace({mbm_local},"channel","$1","__name__",".*_(cha[0-9]+_rmid[0-9]+)_rdt_mbm_local")[5m:])/300,0))', "{{channel}} · D{{DeviceId}}/A{{AccessId}}")
], unit="ops", minimum=0)
advance(9)
add_bar_gauge("Top RDT CMT occupancy", "当前 CHA/RMID cache monitoring technology 值最高的 24 条。", 0, 8, 10,
              f'topk(24,label_replace({cmt},"channel","$1","__name__",".*_(cha[0-9]+_rmid[0-9]+)_rdt_cmt"))', "{{channel}} · D{{DeviceId}}/A{{AccessId}}", "locale", 0)
...
add_timeseries("QAT bandwidth", "QAT0/QAT1 telemetry 的 in/out megabytes 当前值。", 16, 8, 10, [
    target(f'{{__name__=~"qat[01]_tl_.*_bw_(in|out)_megabytes",{sel}}}', "{{__name__}} · D{{DeviceId}}/A{{AccessId}}")
...
add_timeseries("QAT maximum latency", "QAT max read/general-purpose latency。", 12, 12, 8, [
    target(f'{{__name__=~"qat[01]_tl_.*_max_.*_nanoseconds",{sel}}}', "{{__name__}} · D{{DeviceId}}/A{{AccessId}}")
], unit="ns", minimum=0, legend_place="right")
advance(8)
...
add_table("Data-loss detail", "当前 data-loss count 与 timestamp；timestamp 单位为 25MHz crystal-clock TSC ticks。", 12, 12, 10, [
    target(f'agg_data_loss_count_agg_data_loss_count{{{sel}}}', "", "A", True, "table"),
    target(f'agg_data_loss_timestamp_agg_data_loss_timestamp{{{sel}}}', "", "B", True, "table"),
])
advance(10)
...
add_timeseries("Data-loss positive delta by source", "最近五分钟新增 data loss；持续大于 0 时，期间同 aggregator 的其他 samples 可能不可靠。", 12, 12, 10, [
    target(f'clamp_min(delta(agg_data_loss_count_agg_data_loss_count{{{sel}}}[5m]),0)', "D{{DeviceId}}/A{{AccessId}}/S{{SourceId}}")
], unit="locale", minimum=0)
advance(10)

row("07 · Metric Explorer / 全量 4,387 metrics 探索", "顶部 Metric 下拉框支持搜索任意 metric 名称。专用面板覆盖关键语义；Explorer 覆盖全部长尾 metrics。")
...
variables = [
    {"name": "endpoint", "label": "Endpoint", "type": "query", "datasource": DATASOURCE,
     "query": {"query": "label_values({RedfishEndpoint=~\".+\"},RedfishEndpoint)", "refId": "PrometheusVariableQueryEditor-Endpoint"},
     "definition": "label_values({RedfishEndpoint=~\".+\"},RedfishEndpoint)", "refresh": 1, "sort": 1,
     "current": {"selected": True, "text": "avc01", "value": "avc01"}},
    {"name": "device", "label": "Device", "type": "query", "datasource": DATASOURCE,
     "query": {"query": "label_values({RedfishEndpoint=\"$endpoint\"},DeviceId)", "refId": "PrometheusVariableQueryEditor-Device"},
     "definition": "label_values({RedfishEndpoint=\"$endpoint\"},DeviceId)", "refresh": 1, "sort": 3,
     "includeAll": True, "allValue": ".*", "multi": True,
     "current": {"selected": True, "text": ["All"], "value": ["$__all"]}},
    {"name": "access", "label": "Access", "type": "query", "datasource": DATASOURCE,
     "query": {"query": "label_values({RedfishEndpoint=\"$endpoint\",DeviceId=~\"$device\"},AccessId)", "refId": "PrometheusVariableQueryEditor-Access"},
     "definition": "label_values({RedfishEndpoint=\"$endpoint\",DeviceId=~\"$device\"},AccessId)", "refresh": 1, "sort": 3,
     "includeAll": True, "allValue": ".*", "multi": True,
     "current": {"selected": True, "text": ["All"], "value": ["$__all"]}},
    ...
    {"name": "metric", "label": "Metric (search all)", "type": "query", "datasource": DATASOURCE,
     "query": {"query": "label_values({RedfishEndpoint=\"$endpoint\"},__name__)", "refId": "PrometheusVariableQueryEditor-Metric"},
     "definition": "label_values({RedfishEndpoint=\"$endpoint\"},__name__)", "refresh": 1, "sort": 1,
     "current": {"selected": True, "text": "c0_c1_c2_c3_temp_c0_temp_celcius", "value": "c0_c1_c2_c3_temp_c0_temp_celcius"}},
]
...
dashboard = {
    ...
    "description": "Comprehensive Intel PMT Redfish observability: health, thermal, core activity, histograms, memory/CHA, QAT, collection quality, and all-metric exploration.",
    ...
    "tags": ["Intel PMT", "Redfish", "GNR", "hardware telemetry", "comprehensive"],
    ...
    "timezone": "browser", "title": "Intel PMT · Comprehensive Redfish Telemetry", "uid": "pmt-avc01-redfish", "version": 10,
}
